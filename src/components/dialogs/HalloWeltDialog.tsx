...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/HalloWelt';
...
export function HalloWeltDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: HalloWeltDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<HalloWelt['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'hallo_welt');
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'nachricht': (
      <div key="nachricht" className="space-y-1.5">
        ...
        <Textarea
          id="nachricht"
          placeholder="Deine Nachricht, Fragen, Ideen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "nachricht": "Nachricht"};
